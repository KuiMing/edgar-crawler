...
def get_ticker(url, timeout=20):
    """
    Fetch a list of stock ticker symbols from a specified URL

    Args:
        url (str): URL of the webpage containing the stock ticker table
        timeout (int): Request timeout in seconds, defaults to 20 seconds

    Returns:
        list: Sorted list of unique ticker symbols

    Raises:
        RuntimeError: When unable to locate ticker table on the page
    ...
    symbol_column_names = ["symbol", "ticker", "ticker symbol"]
    ...
    # Parse HTML tables
    html_tables = pd.read_html(StringIO(response.text))
    target_table = None

    # Find table containing ticker symbols
    for table in html_tables:
        column_names = [str(col).strip().lower() for col in table.columns]
        if any(col_name in column_names for col_name in symbol_column_names):
            target_table = table
            break

    if target_table is None:
        raise RuntimeError(
            f"Cannot locate table with symbol columns {symbol_column_names} on the page"
        )
    ...
    symbol_column = next(
        col for col in target_table.columns if col.lower() in symbol_column_names
    )

    # Extract and process ticker symbols
    ticker_symbols = target_table[symbol_column].astype(str).str.strip()

    # Remove duplicates and sort
    unique_tickers = ticker_symbols.unique().tolist()
    return sorted(set(unique_tickers))


def get_all_tickers():
    """
    Fetch the list of stocks from various US market indices
    """
    base_url = "https://en.wikipedia.org/wiki/List_of_S%26P_{}_companies"
    indices = [500, 400, 600]
    return sorted(
        set(ticker for idx in indices for ticker in get_ticker(base_url.format(idx)))
    )
